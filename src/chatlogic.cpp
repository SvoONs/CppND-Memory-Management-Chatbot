...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
        ...
        while (getline(file, lineStr)) {
            ...
            while (lineStr.size() > 0) {
                ...
                if (posTokenFront < 0 || posTokenBack < 0)
                    break; // quit loop if no complete token
                           // has been found
                std::string tokenStr =
            ...
            if (type != tokens.end()) {
                ...
                if (idToken != tokens.end()) {
                    ...
                    if (type->second == "NODE") {
                        ...
                        // create new element if ID does
                        // not yet exist
                        if (newNode == _nodes.end()) {
                            _nodes.emplace_back(
                                std::make_unique<GraphNode>(id));
                            newNode = _nodes.end() - 1; // get iterator
                                                        // to last
                                                        // element
                    ...
                    if (type->second == "EDGE") {
                        // find tokens for incoming
                        // (parent) and outgoing (child)
                        // node
                        ...
                        if (parentToken != tokens.end() &&
                            ...
                            // find all keywords for
                            // current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in
                            // child node and parent
                            // node
                    ...
                    std::cout << "Error: ID missing. Line "
                                 "is ignored!"
                              << std::endl;
                }
            }
    ...
    // add pointer to chatbot instance
